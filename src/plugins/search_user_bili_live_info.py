...
async def send_msg(bot: Bot, event: Event, state: T_State):
    ...
    out_str = " 昵称:" + info_json["data"]["channel"]["name"] + "  UID:" + content + "  房间号:" + \
              ...
              " 显示格式为:【 标题 】 弹幕数 | 观看数 | 互动数 | 总收益\n"

    for i in range(len(info_json["data"]["lives"])):
        out_str += "【 {:<s} 】 {:<d} | {:<d} | {:<d} | ￥{:<.1f}".format(info_json["data"]["lives"][i]["title"],
    ...
    if len(info_json["data"]["lives"]) < 1000:
        # img: PIL.Image.Image
        ...
        id = event.get_user_id()
        msg = "[CQ:at,qq={}]".format(id) + '果咩，直播数大于1000，发不出去喵~'
        await catch_str.finish(Message(f'{msg}'))
